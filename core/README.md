...
데이터 베이스 커넥션 풀, 네트워크 소켓처럼 에플리케이션 시작 시점에 필요한 연결을 미리해두고, 종료 시점에연결을 모두 종료하라면 -> **초기화와 종료작업**이 필요하다

##### 스프링 빈의 라이프 사이클
- 스프링 빈의 라이프사이클
  - 객체 생성 -> 의존관계 주입
  - 생성자 주입은 예외!(생성자에서 만들어지면서, 스프링 빈이 들어오기 때문에)
  - 즉 **스프링 빈은 객체를 생성**하고, **의존관계까지 주입이 다 끝난 다음**에야 **필요한 데이터를 사용할 수 있는 준비**가 완료된다
  - setter로 의존관계를 주입해도, 객체가 생성하는 단계에서는 값이 없다...
  - 그래서 **초기화**라는 작업은 객체를 생성하는 작업을 말하는게 아니라, **객체 안에 필요한 값이 다 연결돼있고, 진짜 외부랑 연결해야 할 때**, 객체가 처음 일을 제대로 시작하는 것이다
  - 그래서 **초기화 작업**은 **의존관계 주입까지 모두 완료되고 난 뒤에 호출**해야 한다 -> 안그럼 null값 나옴
  - **객채의 생성하는 부분**과 **초기화하는 부분**을 명확하게 **나눠주는 것**이 **유지보수 관점**에서 **좋다!!!**
    - 생성자 내부에서는 값을 세팅하는 정도만 하자(메모리 할당-> 객체 생성정도,,,)
    - 무거운작업(외부와 연결, 등등 무거운 동작)은 별도의 초기화 메서드로 제공하는게 유지보수 관점에서 좋다
    - 또 나누면 지연도 가능하다! -> 나중에 필요하다면...(그렇게 많이 하지는 않는다.)
  - 그런데 개발자가 의존관계가 주입이 모두 완료된 시점을 어떻게 알아??  -> **3가지 방법이 있다**
 
 1. **인터페이스 InitializingBean, DisposableBean**
  - 인터페이스 InitializingBean, DisposableBean이 있는데 이것들은 각각, 추상 메서드 afterPropertiesSet, destroy를 가지고 있어 오버라이드 해준다
  - afterPropertiesSet(): 의존관계가 주입이 끝나면 호출된다. 이 메서드 안에 초기화 코드를 넣어준다
  - destroy : 스프링 빈이 종료될때 호출된다.
  - 컨테이너의 클로징이 선언되면!!!
  - 스프링 컨테이너가 종료되기 전에, 싱글톤 빈들이 하나씩 죽는다.. ->그 때 destroy가 호출된다
  - 그 후에 disconnet가 안전하게 호출되서, 서버와 연결이 종료되고
  - 스프링 컨테이너가 완전하게 내려간다!!!

2.**빈 등록 초기화, 소멸 메서드**
  - 앞서 인터페이스를 오버라이드 했다면, 이번엔 간단하게 메서드를 만들어준다
  - @Bean(initMethod = "init", destroyMethod = "close")이렇게 빈등록을 해준다
  - 메서드 이름 자유롭게 변경가능
  - 스프링 빈이 **스프링 코드에 의존하지 않는다** -> 테스트에서 **좋은 테스트 코드**를 작성할 수 있다
  - 설정정보를 사용하므로 **외부라이브러레서도 초기화 종료 메서드를 적용**할 수 있다.
  - **@Bean의 destroyMethod**는 아주 특별한 기능이 있다 ->close, shutdown이름의 메서드를 찾아서 자기가 종료 메서드라고 추론한다. 따라서 따로 적어주지 않아도 잘 작동한다.
  - 내가 외부라이브러리를 @Bean으로 의존관계 주입해서 쓴느데, 종료메서드가 자동으로 호출될 수 있다는 말이다
  - 쓰기 싫으면 destroyMethod =""로 빈 공백을 지정하면 된다

3. **애노테이션 @PostConstruct, @PreDestroy**
  - **이방법을 쓰시면 됩니다~(스프링에서도 권고한다)**
  - @PostConStruct, @PreDestory
  - 패키지가 javax.annotation이다
  - javax면 java진영에서 공식적으로 지원하는 것이다. 스프링이 아닌 다른 컨테이너를 사용해도 자바니까 지원이 된다.
  - 그치만 외부 라이브러리에는 적용을 못한다

<정리>
1.**@PostConstruct , @PreDestroy**를 사용하자
2. 코드를 수정할 수 없는 **외부 라이브러리들의 초기화,종료**해야 한다면, **@Bean의 initmethod, destroyMethod**를 사용하자!
